...
class PaymentProvider(models.Model):
    ...
    def _highriskify_load_module_image(self, relative_path):
        try:
            with file_open(f'highriskify_onramp/{relative_path}', 'rb') as image_file:
    ...
    def action_highriskify_cleanup_legacy_tracking_views(self):
        ...
        for view in candidate_views:
            ...
            module_owned = any(xml.module == 'highriskify_onramp' for xml in xml_ids)
    ...
    def action_highriskify_repair_clean_provider_view(self):
        ...
        view = self.env.ref('highriskify_onramp.payment_provider_form_highriskify_onramp', raise_if_not_found=False)
        if not view:
            return True
        arch = """
<data>
...
"""
    ...
    highriskify_checkout_description = fields.Text(
        ...
        default='Please follow the hosted checkout instructions carefully. To avoid transaction failure, do not change values on the payment page. Once the configured checkout workflow reports a completed transaction, the original merchant website will be notified automatically.',
    )
    ...
    def _get_redirect_form_view(self, is_validation=False):
        self.ensure_one()
        if self.code == const.PROVIDER_CODE:
            return self.env.ref('highriskify_onramp.redirect_form')
