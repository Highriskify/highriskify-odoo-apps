...
class PaymentTransaction(models.Model):
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        # The HighRiskify wallet callback follows the WooCommerce plugin shape:
        # callback_url?order_id=<reference>&nonce=<nonce>&txid_out=...&value_coin=...&coin=...
        # `order_id` is mapped to the Odoo payment.transaction reference.
    ...
    def _process_notification_data(self, notification_data):
        ...
        if pending_flag:
            if self.state not in ('pending', 'done'):
                self._set_pending(_('HighRiskify onramp pending/unconfirmed callback received. Waiting for confirmed callback.'))
        ...
        self._set_done(_('HighRiskify onramp payment confirmed. TXID: %s') % (txid_out or txid_in or '-'))
    ...
    def _highriskify_mark_order_payment_started(self, title):
        ...
        try:
            if self.state not in ('pending', 'done'):
                self._set_pending(_('HighRiskify onramp checkout started. Waiting for confirmed callback.'))
        except Exception:
