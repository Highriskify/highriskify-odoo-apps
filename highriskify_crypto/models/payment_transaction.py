...
class PaymentTransaction(models.Model):
    ...
    def _process_notification_data(self, notification_data):
        ...
        if pending_flag:
            if self.state not in ('pending', 'done'):
                self._set_pending(_('HighRiskify Crypto pending/unconfirmed callback received. Waiting for confirmed callback.'))
        ...
        self._set_done(_('HighRiskify Crypto multicoin payment confirmed. TXID Out: %s') % (txid_out or '-'))
    ...
    def _cryptopaymate_mark_order_payment_started(self, title):
        ...
        try:
            if self.state not in ('pending', 'done'):
                self._set_pending(_('Crypto payment session started. Waiting for confirmed HighRiskify Crypto callback.'))
        except Exception:
